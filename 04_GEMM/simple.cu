...
__global__ void matrix_mult_kernel(const float *A, const float *B, float *C, const int N, const int K, const int M) {
...
at::Tensor matrix_mult_simple(at::Tensor A, at::Tensor B) {
    ...
    dim3 number_of_blocks(CEIL_DIV(M, I_TILE_DIM),
        ...
        matrix_mult_kernel<<<number_of_blocks, threads_per_block>>> (
        A.data_ptr<float>(), 
        B.data_ptr<float>(), 
        C.data_ptr<float>(), 
        N, 
