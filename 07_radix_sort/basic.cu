...
std::vector<int> generateRandomIntVector(uint32_t size, int min, int max) {
...
int main() {
    ...
    auto h_input = generateRandomIntVector(N, -100, 100);
    int h_output[N];
    ...
    CUDA_CHECK(cudaMalloc((void**)&d_input_buff, N * sizeof(int)));
    ...
    CUDA_CHECK(cudaMemcpy(d_input_buff, h_input.data(), N * sizeof(int), cudaMemcpyHostToDevice));
    int_to_uint32<<<CEIL_DIVI(N, BLOCK_DIM), BLOCK_DIM>>>(d_input_buff, d_input, N);
    ...
    std::reverse(h_input.begin(), h_input.end());
    for (int i = 0; i < N; ++i) {
